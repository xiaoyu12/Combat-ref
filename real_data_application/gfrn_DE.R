...
for(curr_path in names(path_mapping)){
  ...
  cpm_df_mlt$Condition <- factor(cpm_df_mlt$Condition)
...
for(curr_path in path_mapping){
  ...
  res_onestep <- edgeR_DEpipe(cts=curr_cts, batch=curr_batch, group=curr_group, 
                              include.batch=TRUE, alpha.unadj=unadj.v, alpha.fdr=fdr.v) 
...
df.num_DEgenes <- do.call(rbind, num_DEgenes)
write.csv(df.num_DEgenes, file=file.path(output_dir, "num_DEgenes.csv"))
...
for(curr_path in path_mapping) {
  ...
  for(i in 1:(length(degenes)-1)) {
    for(j in (i+1):length(degenes)) {
      df.overlap[i, j] <- length(intersect(degenes[[i]], degenes[[j]]))
    }
  }
  print(df.overlap)
  write.csv(df.overlap, file=file.path(output_dir, sprintf("num_DEgene_overlap_%s.csv", curr_path)))
}
...
fisher <- sapply(ras_res, function(x){

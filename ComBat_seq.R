...
ComBat_seq <- function(counts, batch, group=NULL, covar_mod=NULL, full_mod=TRUE, 
  ...
  if(full_mod & nlevels(group)>1){
    ...
    mod <- model.matrix(~group)  # model.matrix(~0+group)
  }else{
...
# counts: counts of a batch
# group: groups of a batch
estimate_Batch_CommonDisp <- function(counts, group) {
  group = as.factor(group)
  mod <- model.matrix(~0+group)
  if (nlevels(group) > 1) {
    disp = estimateGLMCommonDisp(counts, design=mod, subset=nrow(counts))
  } else {
    disp = estimateGLMCommonDisp(counts, design=NULL, subset=nrow(counts))
  }
  
  return (disp)
}
ComBat_seq_new <- function(counts, batch, group=NULL, covar_mod=NULL, full_mod=TRUE, 
                           shrink=FALSE, shrink.disp=FALSE, gene.subset.n=NULL,
                           genewise.disp=FALSE) 
  {
  ########  Preparation  ######## 
  ...
  # ncol = ncol(glm_f$coefficients)
  # alpha_g <- glm_f$coefficients[, n_batch:ncol] %*% as.matrix(n_groups/n_sample) #compute intercept as batch-size-weighted average from batches
  # new_offset <- t(vec2mat(getOffset(dge_obj), nrow(counts))) +   # original offset - sample (library) size
  #   vec2mat(alpha_g, ncol(counts))  # new offset - gene background expression # getOffset(dge_obj) is the same as log(dge_obj$samples$lib.size)
  # glm_f2 <- glmFit.default(dge_obj$counts, design=design, dispersion=phi_matrix, offset=new_offset, prior.count=1e-4) 
  # 
...
#adj_counts_combatseq_ebOFF <- ComBat_seq_new(counts=cts, batch=batch, group=group, shrink=FALSE, genewise.disp=TRUE)
#combatseq_sub <- ComBat_seq(counts=cts_sub, batch=batch_sub, group=group_sub, shrink=FALSE)
#combatseq_new <- ComBat_seq_new(counts=cts_sub, batch=batch_sub, group=group_sub, genewise.disp=FALSE)

...
exp_name <- paste0("sim", ifelse(sim_outliers, "_simout", ""), 
                   ...
                   "_biofc", bio_fold, "_depth", coverage)
...
for(ii in seq_along(alpha_fdr_seq)){
  ...
  for(iter in 1:iterations){
    ...
    size_vec = as.numeric(array(size_mat))
    ## simulate data with batch effect
    simulate_experiment(fasta_file, reads_per_transcript=readspertx, size=size_vec, 
    ...
    # - reference batch 1
    size_base_vec = as.numeric(array(size_mat_base))
    simulate_experiment(fasta_file, reads_per_transcript=readspertx, size=size_base_vec, 
    ...
    # Compare with common dispersion for each batch
    #adj_counts_combatseq_comDisp <- ComBat_seq(counts=cts, batch=batch, group=group, shrink=FALSE, genewise.disp=FALSE)
    #de_called6 <- edgeR_DEpipe(counts_mat=adj_counts_combatseq_comDisp, batch=batch, group=group, include.batch=FALSE, alpha.unadj=alpha_unadj, alpha.fdr=alpha_fdr)  
    #de_called6_deseq <- DESeq2_DEpipe(counts_mat=adj_counts_combatseq_comDisp, batch=batch, group=group, include.batch=FALSE, alpha.unadj=alpha_unadj, alpha.fdr=alpha_fdr)
    # Compare with RUVseq 
    ...
    write.table(DE_res_fdr["fpr", ], sprintf('fprADJ_%s.csv', exp_name),
                append=!first.file, col.names=first.file, row.names=FALSE, sep=",") # type 1 error rate (false positive rate)
    write.table(DE_res_fdr["prec", ], sprintf('precADJ_%s.csv', exp_name),
               append=!first.file, col.names=first.file, row.names=FALSE, sep=",") # precision 
